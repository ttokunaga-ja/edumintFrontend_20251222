...
test.describe('Exam Editing Flow - Global Save Only', () => {
  ...
  test('should change question type and show appropriate editor', async ({ page }) => {
    ...
    await expect(page.locator('text=選択肢')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.locator('text=マッチング')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should save entire exam via TopMenuBar', async ({ page }) => {
    ...
    await expect(page.locator('text=保存しました')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should block save when validation fails', async ({ page }) => {
    ...
    await expect(page.locator('text=必須項目です')).toBeVisible({ timeout: 10000 });
