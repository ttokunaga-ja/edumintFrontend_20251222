...
export function MultilingualAutocomplete({
  ...
  const handleInputChange = (newValue: string) => {
    ...
    debounceTimerRef.current = window.setTimeout(() => fetchSuggestions(newValue), 300);
  };
  ...
  const handleFocus = () => {
    setShowSuggestions(true);
    if (inputValue.length >= minChars && suggestions.length === 0) fetchSuggestions(inputValue);
  };
  ...
  const highlightMatch = (text: string, query: string) => {
    ...
    return (
      <>
        {parts.map((part, index) => (regex.test(part) ? <strong key={index}>{part}</strong> : <span key={index}>{part}</span>))}
      </>
    );
  };
  ...
  return (
    <Box ref={containerRef} sx={{ position: 'relative' }}>
      ...
      {showSuggestions && (suggestions.length > 0 || isLoading) && (
        <Paper sx={{ position: 'absolute', zIndex: 1200, left: 0, right: 0, mt: 1 }}>
            ...
            <List dense>
              {suggestions.map((suggestion) => (
                <ListItemButton key={suggestion.id} onClick={() => handleSuggestionClick(suggestion)}>
                  <ListItemText
                    primary={highlightMatch(suggestion.name, inputValue)}
                    secondary={
                      showReadings
                        ? [suggestion.nameKana, suggestion.nameRomaji, suggestion.nameEn].filter(Boolean).join('  ')
                        : undefined
                    }
                  />
                </ListItemButton>
...
import { suggestReadings } from '@/features/search/repository';
import type { ReadingSuggestion } from '@/features/search/models'; export type MultilingualAutocompleteProps = { value: string; onChange: (value: string, item?: ReadingSuggestion) => void; placeholder?: string; category?: 'university' | 'faculty' | 'subject' | 'teacher'; icon?: ReactNode; showReadings?: boolean; // 読みを表示するか minChars?: number; // 最小検索文字数
